...
if [ "$2" = "--local" ]; then
  ...
  docker run -d -p 9090:9090 --rm -e NODE_ENV=t1 --env-file=.env.t1 --name zeitgeist-processor processor
elif [ "$2" = "--t2" ]; then
  docker run -d -p 9090:9090 --rm -e NODE_ENV=t2 --env-file=.env.t2 --name zeitgeist-processor processor
elif [ "$2" = "--m1" ]; then
  docker run -d -p 9090:9090 --rm -e NODE_ENV=m1 --env-file=.env.m1 --name zeitgeist-processor processor
elif [ "$2" = "--m2" ]; then
  docker run -d -p 9090:9090 --rm -e NODE_ENV=m2 --env-file=.env.m2 --name zeitgeist-processor processor
else
