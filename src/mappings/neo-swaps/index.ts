...
export const combinatorialPoolDeployed = async (
  ...
  await Promise.all(
    neoPool.account.balances.map(async (ab, i) => {
      ...
      if (!asset) {
        // Create new Asset record for combinatorial token
        // Extract unique hash from combinatorial token JSON for unique ID
        const hashMatch = ab.assetId.match(/"0x([a-f0-9]+)"/);
        const uniqueId = hashMatch ? hashMatch[1].substring(0, 12) : ab.assetId.replace(/[^a-zA-Z0-9]/g, '').substring(0, 10);
        asset = new Asset({
